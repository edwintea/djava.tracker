...
public class UserService {
    ...
    public User registerUser(UserDTO userDTO, String password, Boolean privileged) {
        User
            .findOneByLogin(userDTO.login.toLowerCase())
            .ifPresent(
                existingUser -> {
        ...
        // new user gets registration key
        newUser.activationKey = null;
        Set<Authority> authorities = new HashSet<>();
        Authority.<Authority>findByIdOptional(privileged ? AuthoritiesConstants.OWNER : AuthoritiesConstants.USER).ifPresent(authorities::add);
        newUser.authorities = authorities;
        User.persist(newUser);
        log.debug("Created Information for User: {}", newUser);
        return newUser;
